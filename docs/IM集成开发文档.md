...
WeKnora 的 IM 集成模块将企业即时通讯平台（企业微信、飞书、Slack、Telegram、钉钉、Mattermost、微信、微信服务号、QQBot）接入 WeKnora 知识问答管道，支持在 IM 中直接向 AI 提问并获得回答。
...
- [平台适配器详解](#平台适配器详解)
  ...
  - [微信服务号 (WeChat Official Account)](#微信服务号-wechat-official-account)
- [斜杠指令系统](#斜杠指令系统)
- [QA 队列与限流](#qa-队列与限流)
- [流式输出机制](#流式输出机制)
- [文件消息处理](#文件消息处理)
...
微信服务号使用 **Webhook** 回调接收用户消息，并通过**客服消息 API** 异步回复。由于微信服务器要求开发者服务器在短时间内响应，WeKnora 会立即返回 `200` 空 body，再在后台执行 QA 并推送客服消息。

> 当前 MVP 支持微信公众平台测试号或认证服务号，使用明文模式，仅处理文本消息。图片、语音、文件、加密模式和菜单能力属于后续增强。

#### 第一步：准备服务号或测试号

1. 登录 [微信公众平台](https://mp.weixin.qq.com/) 或打开测试号页面。
2. 获取：
   - **App ID**
   - **App Secret**
3. 自定义一个 **Token**，后续需同时填入 WeKnora 和微信公众平台服务器配置。
...
2. 填写配置：
   ...
   - **接入模式**：固定为 **Webhook**
   - **输出模式**：固定为 **完整输出**
   - **App ID**：服务号或测试号 AppID
   - **App Secret**：服务号或测试号 AppSecret
   - **Token**：自定义 Token
   - **API Base URL**：可选，默认 `https://api.weixin.qq.com`
3. 保存后编辑该渠道，复制渠道卡片中的**回调地址**。

#### 第三步：配置微信服务器地址

在微信公众平台「服务器配置」中填写：

- **URL**：`https://你的域名/api/v1/im/callback/{channel_id}`
- **Token**：与 WeKnora 渠道凭证中的 `token` 一致
- **EncodingAESKey**：MVP 明文模式不需要
- **消息加解密方式**：选择明文模式

保存时微信会发起 GET 校验；校验通过后，关注测试号或服务号并发送文本消息，应在数秒后收到 WeKnora 的客服消息回复。

#### 注意事项

- 客服消息存在用户交互窗口限制，窗口外无法主动推送。
- 微信重复投递同一 `MsgId` 时，WeKnora 使用 `channelID:MsgId` 去重。
- 回调响应必须为空 body；不要返回 JSON。
...
每个渠道以卡片形式展示，包含：
- **平台标识**：企业微信 / 飞书 / Slack / Telegram / 钉钉 / Mattermost / 微信 / 微信服务号 / QQBot
- **渠道名称**：用户自定义
...
CREATE TABLE im_channels (
    ...
    platform          VARCHAR(20) NOT NULL,       -- 'wecom' | 'feishu' | 'slack' | 'telegram' | 'dingtalk' | 'mattermost' | 'wechat' | 'wechat_mp' | 'qqbot'
...
| 微信服务号 | Webhook（唯一支持） | `app_id`, `app_secret`, `token`; `api_base_url`（可选） |
...
type IncomingMessage struct {
    Platform    Platform          // "wecom" | "feishu" | "slack" | "telegram" | "dingtalk" | "mattermost" | "wechat_mp" ...
...
微信服务号适配器的平台标识为 `wechat_mp`，仅支持 **Webhook** 模式。入站消息由微信服务器以 XML POST 到统一回调地址，出站回复通过微信客服消息 API 异步发送给用户 `openid`。

#### Webhook 模式

```
微信服务器 ──HTTP GET/POST──▶ /api/v1/im/callback/{channel_id}
                                  │
                          Token + timestamp + nonce
                          SHA1 签名校验
                                  │
                          XML text → IncomingMessage
                                  │
                          立即 200 空 body
                                  │
                          后台 QA → 客服消息 API
...
- **URL 验证：** 微信 GET 请求携带 `echostr`，适配器先校验 `signature`，通过后原样返回 `echostr`。
- **安全校验：** 明文模式下使用 `token/timestamp/nonce` 字典序排序拼接后 SHA1，与 query `signature` 常数时间比较；`timestamp` 限制在 5 分钟窗口内。
- **消息格式：** 微信 POST body 是 XML，当前 MVP 仅处理 `MsgType=text`；`event/image/voice/video/location/link` 返回 `nil` 消息，仅 ack。
- **快速 ack：** `IMCallback` 对 `wechat_mp` 返回 `200` 空 body，避免微信将 JSON ack 当作被动回复内容。
- **用户身份：** `FromUserName`（openid）映射为 `IncomingMessage.UserID`，不自动绑定 WeKnora 用户。
- **消息去重：** Service 使用 `channelID:MsgId` 作为去重 key，避免跨渠道 `MsgId` 冲突。

#### 客服消息回复

`SendReply` 会把 WeKnora 的 Markdown 答案降级为微信纯文本，然后按 550 rune 分片调用：

```
POST /cgi-bin/message/custom/send?access_token=...
```

access token 通过 `/cgi-bin/token?grant_type=client_credential&appid=...&secret=...` 获取，并在内存中缓存到过期前 5 分钟。若客服消息返回 `40001` 或 `42001`，适配器会清空 token 缓存并重试一次；`45015`（客服消息窗口外）和 `45009`（限流）只记录 warn，不做放大式重试。

#### 不支持的能力

- 不实现 `StreamSender`：微信客服消息不支持编辑同一条消息，流式输出固定降级为完整输出。
- 不实现 `FileDownloader`：图片、语音、文件消息留到后续阶段。
- 不支持加密模式：MVP 仅支持明文模式。
...
| 文件 | 职责 |
|------|------|
| `internal/im/wechat_mp/adapter.go` | `im.Adapter` 实现：URL 验证、签名校验、XML 解析、发送回复 |
| `internal/im/wechat_mp/client.go` | access token 缓存、客服消息发送、错误码处理 |
| `internal/im/wechat_mp/markdown.go` | Markdown → 微信纯文本降级与分片 |
| `internal/im/wechat_mp/sign.go` | Token 签名计算与校验 |
| `internal/im/wechat_mp/factory.go` | 从 `im_channels.credentials` 构造适配器 |
...
| 微信服务号 | MVP 暂不支持文件下载；图片/语音/视频消息仅 ack |
...
| 微信服务号签名有效期 | 5 min | `timestamp` 超出窗口拒绝回调，降低重放风险 |
| 微信服务号文本分片 | 550 rune | 客服消息文本分片上限，给微信接口限制留余量 |
...
| 微信服务号 5 秒回调限制 | 立即返回 `200` 空 body，后台异步通过客服消息回复 |
| 微信服务号客服窗口外 (`45015`) | 记录 warn，不重试 |
| 微信服务号客服消息限流 (`45009`) | 记录 warn，不立即重试 |
| 微信服务号 access token 失效 (`40001`/`42001`) | 清空 token 缓存并重试一次 |
