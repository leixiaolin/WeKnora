...
/example\.(net|org)/`,
  ...
  agentEditor: {
    ...
    im: {
      title: 'Интеграция IM',
      description: 'Подключите агента к платформам мгновенных сообщений, таким как Feishu, Slack, Telegram, DingTalk, Mattermost, WeChat Official Account и QQBot',
      wecom: 'WeCom',
      feishu: 'Feishu',
      slack: 'Slack',
      telegram: 'Telegram',
      ...
      wechatMPConsole: 'WeChat Official Accounts Platform',
      wechatMPModeHint: 'WeChat Official Account всегда использует Webhook и полный ответ. После сохранения скопируйте callback URL в настройки сервера WeChat.',
      wechatMPHint: 'Поддерживаются sandbox test account и verified service account. MVP обрабатывает только текстовые сообщения и отвечает асинхронно через customer-service messages.',
      wechatMPTokenHint: 'Должен точно совпадать с Token в настройках сервера WeChat. Используется для проверки подписи callback.',
      wechatMPAPIBaseURLHint: 'Необязательно. Оставьте пустым для https://api.weixin.qq.com. Private proxy URL должен использовать HTTPS и пройти backend SSRF checks.',
      wechatMPCredentialsRequired: 'Введите App ID, App Secret и Token для WeChat Official Account',
